...
\begin{figure}[H]
    ...
    \node[database, right=2.4cm of bertweet] (db) {
        ChromaDB\\
        证据库
    };
    ...
    \draw[arrow] (agree) -- node[left, font=\scriptsize] {是} (unified);
    \draw[arrow] (agree) -- node[right, font=\scriptsize] {否} (divergent);
...
数据处理模块负责读取\code{train.csv}和\code{val.csv}，并完成文本清洗、标签校验、重复样本和冲突样本处理。\code{load\_datasets()}调用\code{\_clean\_split()}分别清洗训练集与验证集，并额外删除训练集与验证集之间的重叠文本，降低数据泄漏风险。\code{TweetDataset}负责将清洗后的文本封装为PyTorch数据集，在\code{\_\_getitem\_\_()}中调用tokenizer生成\code{input\_ids}、\code{attention\_mask}和\code{labels}，供模型训练与验证使用。

\subsubsection{模型定义与推理 —— \code{src/model.py}、\code{src/predict.py}}

模型基于Hugging Face的\code{AutoModelForSequenceClassification}实现，分类头输出2类概率。\code{model.py}采用懒加载方式，在首次调用\code{classify\_statement()}时加载\code{checkpoints/bertweet/best\_model/}中的最佳模型和分词器，并切换到\code{eval()}模式。推理时系统先对输入文本进行规范化和编码，再在\code{torch.no\_grad()}下前向传播，对logits做softmax，返回预测标签和置信度。\code{predict.py}提供命令行预测接口，便于离线测试。
...
RAG模块用于为LLM解释提供参考证据。\code{retrieve\_rag\_evidence()}动态调用\code{RAG/chroma\_retriever.py}中的\code{ChromaRetriever}，从本地ChromaDB中检索Top-k相似证据，并返回来源、标签、距离和文本内容。RAG不直接参与BERTweet分类；若数据库缺失或依赖异常，函数返回空列表，保证主检测流程仍可运行。
...
\code{app.py}使用FastAPI组织完整流程。\code{/analyze}端点依次调用BERTweet分类、RAG检索、LLM独立判断、结果比较和根因分析，并将ML Verdict、LLM Verdict、Confidence、Reasoning、Comparison和RAG Evidence传入Jinja2模板。前端页面最终展示检测结果、解释文本和检索证据，如图\ref{fig:frontend}所示。
