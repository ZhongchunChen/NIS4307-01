...
本项目以 \code{train.csv} 为基础训练数据，并将公开数据集清洗、去重和标签格式统一后汇总为 \code{datasets/all\_extra.csv}，作为补充训练数据以扩大样本覆盖范围、提高模型泛化能力；模型仍在 \code{val.csv} 上验证。根据本次训练的清洗报告，原始训练集清洗后保留 2832 条样本，\code{all\_extra.csv} 包含 17170 条补充样本；完成补充数据合并与去重后的最终训练集共 20201 条（非谣言 8259 条、谣言 11942 条）。验证集共 401 条（非谣言 226 条、谣言 175 条）。扩展后的训练集规模显著增加，谣言类占比约 59.1\%，存在一定类别差异但无严重长尾。训练配置为学习率 $1\times10^{-5}$、batch size 32、最大训练 16 个 epoch，并以验证集 macro-F1 作为 early stopping 指标，patience=2，即连续两个 epoch 未提升时停止训练。

实验结果显示，模型在第 8 个 epoch 取得最佳验证集表现：accuracy 为 0.9377，macro-F1 为 0.9366，验证集 loss 为 0.2403。第 9、10 个 epoch 的 macro-F1 均未超过第 8 轮，早停机制因此在第 10 个 epoch 后触发，系统最终保存第 8 轮 macro-F1 最优的模型权重作为最终检测模型。

图\ref{fig:accuracy_curve}、图\ref{fig:f1_curve}与图\ref{fig:loss_curve}分别展示验证集 accuracy、macro-F1 以及训练/验证 loss 随 epoch 的变化。从曲线可见，accuracy 与 macro-F1 在前 6 轮总体持续上升，第 7 轮短暂回落后于第 8 轮达到最佳，其中第 8 轮 accuracy 为 0.9377、macro-F1 为 0.9366。训练 loss 从第 1 轮的 0.5469 持续下降至第 10 轮的 0.1128，说明模型不断拟合训练集；验证 loss 在第 6 轮降至最低值 0.2051，第 8 轮为 0.2403，随后在第 9、10 轮分别升至 0.2944 和 0.2926。训练 loss 与验证 loss 的差距后期扩大，表明模型开始出现过拟合。因此，采用 early stopping 能够保留 macro-F1 峰值处的模型，避免继续训练导致泛化性能下降。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/accuracy_curve.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{img/macro_f1_curve.png}
...
最佳模型在验证集上的混淆矩阵如表\ref{tab:confusion}所示。模型对非谣言类（label 0）的精确率为0.9427、召回率为0.9469、F1为0.9448；对谣言类（label 1）的精确率为0.9310、召回率为0.9257、F1为0.9284。结果表明，模型对两类样本均具有稳定的识别能力，且两类的精确率与召回率较为接近。误判方面，12例非谣言被误判为谣言，13例谣言被误判为非谣言。
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        \multirow{2}{*}{\textbf{真实标签}} & \textbf{0（非谣言）} & 214 & 12 \\
        \cline{2-4}
        & \textbf{1（谣言）} & 13 & 162 \\
        \hline
    \end{tabular}
    \caption{最佳模型（Epoch 8）验证集混淆矩阵}
    \label{tab:confusion}
\end{table}

综上，引入公开扩展数据并完成清洗和格式统一后，微调后的BERTweet模型在验证集上的accuracy和macro-F1均达到0.94左右。扩展数据增加了训练样本的规模与来源多样性，早停策略则在后期验证损失回升时保留了性能最优的模型，使模型在两类样本的精确率与召回率之间保持了较好的平衡。
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        \hline
        陈忠淳 & 项目统筹、小模型训练与评估 & GitHub 仓库管理、BERTweet微调与评估、公开数据集清洗与加入训练 \\
